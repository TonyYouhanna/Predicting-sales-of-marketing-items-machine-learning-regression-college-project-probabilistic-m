...
def Pre_processing(df_train, df_test):
    ...
    newdf_train_preprocessed = pd.concat([newdf_train2, X5_train, X11_train], axis=1)
    newdf_test_preprocessed = pd.concat([newdf_test2, X5_test, X11_test], axis=1)

    newdf_train_preprocessed.reset_index(inplace=True)
    newdf_test_preprocessed.reset_index(inplace=True)

    newdf_train_preprocessed.drop(['index'], axis=1, inplace=True)
    newdf_test_preprocessed.drop(['index'], axis=1, inplace=True)
    return newdf_train_preprocessed, newdf_test_preprocessed
newdf_train3, newdf_test3 = Pre_processing(train_df, test_df)
# numeric = ['X2', 'X4', 'X6']
...
# y_test_CB_MinMax.to_csv('CatBoost_result_MinMax_more_than_5.csv', index=False)
